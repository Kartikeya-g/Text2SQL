...
data_analyst_template = """You are a data analyst. You are working on a database named customers, which has a table named customers and the table has the following columns,customer_id INT PRIMARY KEY,
    ...
    city VARCHAR(50),
    join_date DATE
    \n Your job is to perform the given task {prompt} by converting the prompt into sql queries.The result should be the sql query in single line string format excluding. 
    Result: 
    """
